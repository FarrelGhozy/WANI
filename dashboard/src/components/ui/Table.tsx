        ...
        <thead>
          <tr className="border-b border-stone-100 bg-stone-50">
            {columns.map((col) => (
              <th
                key={col.key}
                className={`px-4 py-3 text-xs font-medium uppercase tracking-wider text-stone-500 ${col.headerClass ?? ""}`}
              >
                {col.header}
              </th>
            ))}
            ...
            data.map((item) => (
              ...
              >
                {columns.map((col) => (
                  <td
                    key={col.key}
                    className={`px-4 py-3 text-sm text-stone-700 ${col.className ?? ""}`}
                  >
                    {col.render
                      ? col.render(item)
