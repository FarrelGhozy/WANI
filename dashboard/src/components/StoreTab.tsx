  ...
  for (const part of parts) {
    ...
    if (liburMatch) {
      ...
      if (name.includes("–")) {
        ...
        if (si >= 0 && ei >= 0)
          for (let k = si; k <= ei; k++)
            result[DAYS[k]] = { open: false, start: "", end: "" };
      } else if (isDay(name)) {
    ...
    const timeMatch = part.match(
      /^([A-Za-z]+(?:\s*–\s*[A-Za-z]+)?)\s+(\d{2}:\d{2})–(\d{2}:\d{2})$/
    );
    if (timeMatch) {
      ...
      if (name.includes("–")) {
        ...
        if (si >= 0 && ei >= 0)
          for (let k = si; k <= ei; k++)
            result[DAYS[k]] = { open: true, start, end };
      } else if (isDay(name)) {
    ...
    <div className="space-y-1">
      {DAYS.map((day) => {
        ...
        return (
          <div
            key={day}
            className="flex items-center gap-x-1.5 gap-y-1.5 flex-wrap"
          >
            <span className="w-10 text-xs font-medium text-stone-700 sm:w-14">
              {day}
            </span>
            <button
              type="button"
            ...
            {h.open && (
              <span className="flex items-center gap-x-1.5">
                <TimeSelect
                  value={h.start}
                  onChange={(v) => setDay(day, { start: v })}
                />
                <span className="text-xs text-stone-400">&ndash;</span>
                <TimeSelect
                  value={h.end}
                  onChange={(v) => setDay(day, { end: v })}
                />
              </span>
            )}
          </div>
...
export default function StoreTab() {
  ...
  const isDirty = useMemo(() => {
    ...
    const ref = s ?? blank;
    return (
      form.businessName !== ref.businessName ||
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast(
        store ? "Pengaturan toko berhasil disimpan" : "Toko berhasil dibuat",
        "success"
      );
    } catch {
  ...
  const initial = (store?.businessName || form.businessName || "T")
    .charAt(0)
    .toUpperCase();
          ...
          <div className="mb-6 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500">
                Isi profil toko Anda untuk memulai
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 md:gap-5">
            ...
            <Field label="Jam Operasional" error={errors.businessHours}>
              <BusinessHoursEditor
                ...
                onChange={(v) => {
                  setForm((prev) => ({ ...prev, businessHours: v }));
  ...
  return (
    <>
        ...
        <div className="mb-4 flex flex-col items-center gap-3 sm:flex-row md:mb-6 md:gap-4">
          ...
          <div className="text-center sm:text-left">
            ...
            <p className="text-xs text-stone-500">
              Upload foto profil untuk toko Anda
            </p>
            <button
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:gap-5">
          ...
          <Textarea
            ...
            onChange={(e) =>
              setForm((prev) => ({
                ...prev,
                shippingInfo: e.target.value || null,
              }))
            }
        ...
        <div className="mt-5 flex items-center justify-between gap-3 rounded-lg bg-stone-50 px-3 py-2.5 md:mt-6 md:px-4 md:py-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-stone-500 truncate">
              Nonaktifkan untuk menyembunyikan toko dari pelanggan
            </p>
          </div>
          <button
      ...
      {/* Payment Methods */}
      <Card accent="teal">
        <h2 className="mb-4 text-base font-semibold text-stone-900 md:mb-5 md:text-lg">
          Metode Pembayaran
        </h2>
        <PaymentTab />
      </Card>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500">
                {categories.length} kategori — atur pengelompokan produk Anda
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500">
                Atur tampilan website toko Anda — hero, warna, produk, dan
                template
              </p>
            </div>
          </div>
