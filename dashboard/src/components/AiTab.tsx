...
export default function AiTab({ config, onUpdate }: AiTabProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setDirty(false);
    try {
      await onUpdate({ model, maxTokens, temperature, greetingMessage: greetingMessage || null, systemPrompt, knowledgeBase: knowledgeBase || null });
    } catch {
      setDirty(true);
    } finally {
      setSaving(false);
    ...
    <Card accent="amber">
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:gap-5">
        <Field label="Model">
          ...
          <p className="mt-1 text-xs text-stone-400">
            ...
            Contoh: <code className="text-stone-500">deepseek-v4-flash-free</code>, <code className="text-stone-500">north-mini-code-free</code>
          </p>
        ...
        <div className="sm:col-span-2">
          <Field label="System Prompt">
            ...
            <p className="text-xs text-stone-400">
              Instruksi tambahan untuk AI. Jika dikosongkan, AI akan menggunakan prompt default yang mencakup info toko, katalog produk, dan aturan keamanan.
            </p>
          </Field>
        </div>
        <div className="sm:col-span-2">
          <Field label="Basis Pengetahuan">
            ...
            />
            <p className="text-xs text-stone-400">
              Informasi tambahan yang akan disisipkan ke prompt AI. Bisa diisi jam operasional, daftar promo, FAQ, atau kebijakan khusus toko.
            </p>
          </Field>
        </div>
      </div>
      <div className="mt-5 flex items-center justify-between gap-3 rounded-lg bg-stone-50 px-3 py-2.5 md:mt-6 md:px-4 md:py-3">
        ...
        <button
          ...
          disabled={saving}
          className={`relative h-6 w-11 rounded-full transition-colors ${config.isActive ? "bg-amber-500" : "bg-stone-300"} ${saving ? "cursor-not-allowed opacity-50" : ""}`}
        >
          <span className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${config.isActive ? "translate-x-5" : "translate-x-0"}`} />
        </button>
      </div>
