...
export default function PaymentTab() {
  const { methods, loading, create, update, remove, toggleActive } =
    usePaymentMethods();
  ...
  function validate(): boolean {
    ...
    if (t === "QRIS" && !form.qrImageUrl)
      errs.qrImageUrl = "QR Code wajib diupload";
    if (t === "BANK_TRANSFER") {
    ...
    if (t === "COD" && !form.instructions)
      errs.instructions = "Instruksi pembayaran wajib diisi";
    setErrors(errs);
  ...
  function methodLabel(m: StorePaymentMethod): string {
    ...
    switch (m.type) {
      case "QRIS":
        return "QRIS";
      case "BANK_TRANSFER":
        return `${m.bankName ?? "Bank"} a/n ${m.accountName ?? "-"}`;
      case "E_WALLET":
  ...
  function methodDetail(m: StorePaymentMethod): string {
    switch (m.type) {
      case "BANK_TRANSFER":
        return `${m.bankName} — ${m.accountNumber}`;
      case "E_WALLET":
    ...
    <>
      ...
      <div className="flex items-center justify-between gap-3">
            ...
            {TYPE_ORDER.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="hidden sm:inline ml-0.5 rounded-full bg-stone-200 px-1.5 py-0.5 text-[10px] tabular-nums text-stone-500">
                    {count}
                  </span>
                </button>
              );
        ...
        <Button size="sm" onClick={openCreate}>
          Tambah {TYPE_CONFIG[activeType].label}
        </Button>
      </div>
      ...
      {methods.length === 0 ? (
        <div className="mt-4 rounded-lg border-2 border-dashed border-stone-300 p-6 text-center md:p-8">
          ...
          <p className="text-xs text-stone-400">
            Tambahkan minimal satu metode agar pelanggan bisa membayar
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm table-fixed">
            ...
            <thead>
              <tr className="border-b border-stone-200 text-left text-xs font-medium uppercase tracking-wider text-stone-400">
                ...
                <th className="py-2 pr-3 text-center md:py-3 md:pr-4">
                  Status
                </th>
                ...
                >
                  <td className="py-2 pr-3 md:py-3 md:pr-4">
                    <div className="flex items-center gap-2 md:gap-3">
                      <div
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs md:h-8 md:w-8 md:text-sm ${TYPE_CONFIG[method.type].color}`}
                      >
                  ...
                  <td className="py-2 pr-3 md:py-3 md:pr-4">
                    <div className="flex items-center gap-2 md:gap-3">
                      {method.type === "QRIS" && method.qrImageUrl ? (
                        <img
                          src={mediaUrl(method.qrImageUrl)}
                          alt="QRIS"
                          className="h-8 w-8 rounded border border-stone-200 object-cover md:h-10 md:w-10"
                        />
                      ) : null}
                      <span className="text-[10px] text-stone-600 md:text-xs">
                        {methodDetail(method)}
                      </span>
                    </div>
                  </td>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-xs text-stone-400">
                  Upload gambar QRIS (PNG/JPEG, max 2MB)
                </span>
              </div>
              {errors.qrImageUrl && (
                <p className="text-xs text-red-500">{errors.qrImageUrl}</p>
              )}
            </div>
          )}
