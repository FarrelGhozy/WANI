...
interface ProductListViewProps {
  ...
  sortDir: "asc" | "desc";
  onSort: (
    field: "name" | "category" | "price" | "stock" | "isAvailable"
  ) => void;
}
        ...
        <tbody>
          {products.map((item) => (
            ...
            >
              ...
              <td
                className={`max-sm:px-2 max-sm:py-2 sm:px-4 sm:py-3 text-center tabular-nums ${item.stock === 0 ? "text-red-500" : "text-stone-700"}`}
              >
                {item.stock}
              </td>
              <td className="max-sm:px-2 max-sm:py-2 sm:px-4 sm:py-3">
