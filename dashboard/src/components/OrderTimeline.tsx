    ...
    <div className="space-y-0">
      {steps.map((step, i) => (
        <div key={step.status} className="relative flex gap-4 pb-6 last:pb-0">
          ...
          {i < steps.length - 1 && (
            <div
              className={`absolute left-[11px] top-5 h-full w-0.5 ${step.done ? "bg-teal-200" : "bg-stone-200"}`}
            />
          )}

          {/* Dot */}
          <div
            className={`relative z-10 mt-1 flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full ${
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <p
              className={`text-sm ${step.done ? "font-medium text-stone-900" : "text-stone-400"}`}
            >
              {step.label}
            </p>
            {step.time && (
