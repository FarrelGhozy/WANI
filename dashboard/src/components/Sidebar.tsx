...
const navItems = [
  ...
  { to: "/app/settings", icon: CogIcon, label: "Pengaturan" },
];
  ...
  return (
    <aside className="fixed left-0 top-0 z-30 hidden h-screen w-64 flex-col bg-teal-800 lg:flex">
      ...
      <div className="flex h-16 items-center gap-3 border-b border-teal-700/50 px-6">
        ...
        <div>
          <span className="text-lg font-semibold tracking-tight text-teal-50">
            WANI
          </span>
          <span className="ml-2 rounded bg-teal-700/50 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-teal-200">
            Dashboard
          </span>
        </div>
      </div>
        ...
        {navItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span
                  className={`shrink-0 ${isActive ? "text-teal-50" : "text-teal-300"}`}
                >
                  <item.icon />
                </span>
      ...
      <div className="border-t border-teal-700/50 px-6 py-4">
        <div className="flex items-center gap-3">
          <div className="relative">
            <span
              className={`block h-2 w-2 rounded-full ${statusColor(connection)}`}
            />
            <span
              className={`absolute inset-0 h-2 w-2 animate-ping rounded-full ${statusColor(connection)} opacity-40`}
            />
          </div>
