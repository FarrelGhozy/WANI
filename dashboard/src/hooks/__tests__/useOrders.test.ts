...
describe("useOrders", () => {
  ...
  describe("initial load + mapping", () => {
    it("fetches and maps orders correctly", async () => {
      ...
      expect(result.current.allOrders[0].items[0].productName).toBe(
        "Nasi Goreng"
      );
  ...
  describe("confirmPayment", () => {
    it("confirms payment and updates state", async () => {
      ...
      await act(async () => {
        await result.current.confirmPayment("order-001", {
          method: "TRANSFER",
          amount: 75000,
        });
      });
      ...
      expect(mockFetchApi).toHaveBeenCalledWith(
        "/orders/order-001/payment",
        expect.any(Object)
      );
      expect(result.current.getOrder("order-001")!.payment).toBeDefined();
      expect(result.current.getOrder("order-001")!.payment!.status).toBe(
        "PAID"
      );
    });
  });
