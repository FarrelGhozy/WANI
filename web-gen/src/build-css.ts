...
for (const [t, src] of Object.entries(SRC_MAP)) {
  ...
  const configMatch = raw.match(
    /tailwind\.config\s*=\s*\{[\s\S]*?theme\s*:\s*\{[\s\S]*?extend\s*:\s*\{/
  );
  if (!configMatch) {
...
`;
  ...
  writeFileSync(
    join(inputDir, "_known.html"),
    `<div class="${allRefs.join(" ")}"></div>`
  );
  ...
  const r = spawnSync(
    "bunx",
    ["@tailwindcss/cli", "-i", join(inputDir, "input.css"), "-o", outFile],
    {
      cwd: inputDir,
  ...
  if (r.status !== 0) {
    console.error(
      `[${t}] tailwind build failed:`,
      ((r.stderr || r.stdout)?.toString() || "no output").slice(0, 200)
    );
  } else {
