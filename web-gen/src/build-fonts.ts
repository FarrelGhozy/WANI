...
for (const [, spec] of SPECS) {
  ...
  if (!resp.ok) {
    console.warn(`fetch failed for ${spec}: ${resp.status}`);
    continue;
  }
  ...
  while ((m = regex.exec(css))) {
    ...
    if (!downloaded.has(name) && !existsSync(local)) {
      ...
      if (!fr.ok) {
        console.warn(`download failed for ${name}: ${fr.status}`);
        continue;
      }
...
console.log(
  `fonts.css — ${blocks.length} @font-face rules, ${downloaded.size} files`
);
