...
async function handleOrder(
  ...
  if (unmatched.length > 0) {
    ...
    return {
      reply: `Maaf, produk ${list} tidak ditemukan di katalog kami. Bisa cek nama produknya kembali?`,
    };
  }

  const { order } = await OrderModel.createFromItems(
    ctx.ownerId,
    ctx.customerId,
    resolved,
    output.notes
  ...
  await ActivityLogModel.log(
    ctx.ownerId,
    "order_created",
    `Order ${order.id} created from WA chat`,
    order.id,
    {
      items: resolved,
      notes: output.notes,
  ...
  const lines = resolved.map(
    (r) =>
      `• ${r.productName} × ${r.qty} = Rp${(r.unitPrice * r.qty).toLocaleString("id-ID")}`
  );
...
async function handleInquiry(
  output: Extract<LLMOutput, { intent: "inquiry" }>
): Promise<ActionResult> {
  if (output.reply) {
    return { reply: output.reply };
  }
...
async function handleEscalate(
  ...
  return {
    reply:
      "Baik, akan kami hubungkan dengan CS manusia. Mohon tunggu sebentar ya.",
  };
}
