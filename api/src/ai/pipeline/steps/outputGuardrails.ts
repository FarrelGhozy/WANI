...
async function runOutputGuardrails(
  ...
  if (outputResult.blocked) {
    await ActivityLogModel.log(
      ownerId,
      "output_blocked",
      `Output scan: ${outputResult.reason}`,
      convId,
      {
  ...
  if (piiFound.length > 0) {
    trace.set(
      "pii_redacted",
      piiFound.map((m) => m.type)
    );
    await ActivityLogModel.log(
      ownerId,
      "pii_output",
      `PII in outbound reply: ${piiFound.map((m) => m.type).join(", ")}`,
      convId,
      {
        piiTypes: piiFound.map((m) => m.type),
        intent,
      }
    );
    for (const m of piiFound.sort((a, b) => b.start - a.start)) {
      finalReply =
        finalReply.slice(0, m.start) +
        `[${m.type.toUpperCase()}]` +
        finalReply.slice(m.end);
    }
  }
  ...
  if (intent === "inquiry" || intent === "order") {
    ...
    trace
      .set("grounded", grounding.grounded)
      .set("unsupported_claims", grounding.unsupportedClaims);
    ...
    if (!grounding.grounded) {
      ...
      return (
        "Maaf, ada informasi yang kurang tepat dari jawaban saya sebelumnya. " +
