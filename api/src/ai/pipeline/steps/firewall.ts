...
async function runInputFirewall(ctx: {
  ...
  if (verdict === "BLOCK") {
    await ActivityLogModel.log(
      ctx.ownerId,
      "injection_blocked",
      `T1 blocked: ${scanResult.reasons.join(", ")}`,
      ctx.conversationId!,
      {
        text: ctx.normalized,
  ...
  if (verdict === "UNCERTAIN") {
    ctx.trace
      .set("reason_triggered", scanResult.reasons)
      .begin("firewall_tier2");
    ...
    if (classifierResult.verdict === "INJECTION") {
      await ActivityLogModel.log(
        ctx.ownerId,
        "injection_blocked",
        `T2 blocked: ${classifierResult.reasons.join(", ")}`,
        ctx.conversationId!,
        {
          text: ctx.normalized,
    ...
    if (classifierResult.verdict === "SUSPICIOUS") {
      ...
      const history = await MessageModel.recentByConversation(
        ctx.conversationId!,
        6
      );
      const historyTexts = history.map((m) => `${m.role}: ${m.content}`);

      const judgeResult = await judgeInput(
        ctx.normalized,
        classifierResult.reasons,
        historyTexts
      );
      ctx.trace
        .set("judge_verdict", judgeResult.verdict)
        .set("judge_reasons", judgeResult.reasons);

      if (judgeResult.verdict === "BLOCK") {
        await ActivityLogModel.log(
          ctx.ownerId,
          "injection_blocked",
          `T3 blocked: ${judgeResult.reasons.join(", ")}`,
          ctx.conversationId!,
          {
            text: ctx.normalized,
      ...
      logger.info("Judge passed suspicious message", {
        reasons: judgeResult.reasons,
      });
    }
  }
