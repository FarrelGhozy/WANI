...
setInterval(() => {
  ...
  for (const [key, b] of buckets) {
    const recent =
      b.long.length > 0 && now - b.long[b.long.length - 1]! < longWindow;
...
export function checkRateLimit(key: string): RateLimitResult {
  const now = Date.now();
  const { rateShortMax, rateShortWindowMs, rateLongMax, rateLongWindowMs } =
    env.guardrails;
  ...
  const blocked =
    b.short.length >= rateShortMax || b.long.length >= rateLongMax;
  if (blocked) {
