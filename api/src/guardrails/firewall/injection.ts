...
const MANY_SHOT_RE =
  /^(?:Q:|A:|User:|Assistant:|Human:|AI:|Example|Question|Answer|\d+\.)/i;
...
export function classifyVerdict(reasons: string[]): ScanVerdict {
  ...
  if (reasons.some((r) => HIGH_CONFIDENCE.has(r) || MEDIUM_CONFIDENCE.has(r)))
    return "BLOCK";
  return "UNCERTAIN";
}
...
export function scanInput(text: string): ScanResult {
  ...
  for (const re of OVERRIDE_PATTERNS) {
    if (re.test(normalized)) {
      reasons.push("instruction_override");
      break;
    }
  }
  ...
  for (const re of EXTRACTION_PATTERNS) {
    if (re.test(normalized)) {
      reasons.push("prompt_extraction");
      break;
    }
  }
