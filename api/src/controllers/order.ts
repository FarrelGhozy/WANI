...
export async function listOrders(
  ...
  const result = await OrderModel.list(
    ownerId,
    getValidatedQuery<OrderQuery>(req)
  );
...
const STATUS_MSGS: Record<string, string> = {
  CONFIRMED:
    "✅ Pesanan sudah dikonfirmasi. Kami akan segera memproses pesanan Anda. Terima kasih!",
  PROCESSING:
    "⏳ Pesanan sedang kami proses. Kami akan kabari jika sudah selesai.",
  COMPLETED:
    "🎉 Pesanan sudah selesai! Terima kasih telah berbelanja dengan kami. Silakan datang kembali!",
...
export async function updateOrderStatus(
  ...
  const order = await OrderModel.updateStatus(
    req.params.id,
    req.body.status as $Enums.OrderStatus
  );

  const statusMsg = STATUS_MSGS[order.status];
  if (statusMsg) {
    const conv = await ConversationModel.findOrCreateActive(
      ownerId,
      order.customerId
    );
  ...
  if (order.status === "CONFIRMED") {
    ...
    await MessageModel.append({
      ...
      content: buildNotification(
        order,
        "✅ Pembayaran sudah dikonfirmasi! Pesanan Anda sedang kami proses."
      ),
    });
    await ConversationModel.touch(conv.id);
  }
