...
export async function updateWebsiteConfig(
  ...
  const row = await WebSiteModel.upsertByOwner(
    ownerId,
...
export async function generateWebsite(
  ...
  const selectedIds: string[] = getConfigValue(
    config,
    "selectedProductIds",
    []
  );
  ...
  sendResponse(res, 200, "website generated", {
    slug,
...
export async function downloadWebsite(
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="website-${gen.slug}.zip"`
  );
