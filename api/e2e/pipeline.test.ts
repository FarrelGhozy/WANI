...
const mockComplete = mock().mockImplementation(async () => ({
...
const mockHandleIntent = mock().mockImplementation(async () => ({
  reply: "Halo! Ada yang bisa kami bantu?",
  qrisImageUrl: null,
}));

const mockNormalizeInput = mock((text: string) => text.trim());
const mockScanPii = mock(
  () => [] as { type: string; value: string; start: number; end: number }[]
);
...
describe("AI Pipeline Integration", () => {
  ...
  test("LLM failure — circuit breaker returns fallback", async () => {
    mockWithCircuit.mockImplementationOnce(
      async <T>(
        _fn: () => Promise<T>
      ): Promise<{ allowed: false; error: Error }> => {
