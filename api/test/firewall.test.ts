...
} from "@/src/guardrails/firewall/context";
...
describe("detectObfuscation", () => {
  test("detects base64-encoded payloads", () => {
    const longBase64 =
      "VGhpcyBpcyBhIHZlcnkgbG9uZyBiYXNlNjQgc3RyaW5nIHRoYXQgc2hvdWxkIGJlIGRldGVjdGVkIGFzIGEgcG90ZW50aWFsIG9iZnVzY2F0aW9u";
...
describe("classifyVerdict", () => {
  ...
  test("BLOCK when mixed with high-confidence reason", () => {
    expect(classifyVerdict(["crescendo_marker", "delimiter_escape"])).toBe(
      "BLOCK"
    );
  });
});
...
describe("scanOutput", () => {
  ...
  test("detects delimiter leak", () => {
    const result = scanOutput(
      "<customer_message> some text </customer_message>"
    );
    expect(result.blocked).toBe(true);
