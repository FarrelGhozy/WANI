...
describe("forgotPasswordSchema", () => {
  test("validates email", async () => {
    expect(
      (await forgotPasswordSchema.safeParseAsync({ email: "a@b.com" })).success
    ).toBe(true);
    expect(
      (await forgotPasswordSchema.safeParseAsync({ email: "bad" })).success
    ).toBe(false);
  });
});
...
describe("resetPasswordSchema", () => {
  test("validates token + password", async () => {
    expect(
      (
        await resetPasswordSchema.safeParseAsync({
          token: "abc",
          password: "newpass123",
        })
      ).success
...
describe("productQuerySchema", () => {
  ...
  test("validates isAvailable as enum string", async () => {
    ...
    const r2 = await productQuerySchema.safeParseAsync({
      isAvailable: "false",
    });
...
describe("createCategorySchema", () => {
  test("validates name required", async () => {
    expect(
      (await createCategorySchema.safeParseAsync({ name: "Minuman" })).success
    ).toBe(true);
...
describe("updateCategorySchema", () => {
  test("allows partial update", async () => {
    expect(
      (await updateCategorySchema.safeParseAsync({ name: "Baru" })).success
    ).toBe(true);
...
describe("updateOrderStatusSchema", () => {
  test("validates valid statuses", async () => {
    for (const s of [
      ...
      const result = await updateOrderStatusSchema.safeParseAsync({
        status: s,
      });
      expect(result.success).toBe(true);
    }
...
describe("updateOrderNotesSchema", () => {
  test("validates notes required", async () => {
    expect(
      (await updateOrderNotesSchema.safeParseAsync({ notes: "tolong cepat" }))
        .success
    ).toBe(true);
    expect((await updateOrderNotesSchema.safeParseAsync({})).success).toBe(
      false
    );
  });
});
...
describe("updateCustomerSchema", () => {
  test("validates name or notes", async () => {
    expect(
      (await updateCustomerSchema.safeParseAsync({ name: "Baru" })).success
    ).toBe(true);
    expect(
      (await updateCustomerSchema.safeParseAsync({ notes: "pelanggan tetap" }))
        .success
    ).toBe(true);
    expect(
      (await updateCustomerSchema.safeParseAsync({ name: "" })).success
    ).toBe(false);
  });
});
...
describe("updateConversationStatusSchema", () => {
  test("validates valid statuses", async () => {
    for (const s of ["ACTIVE", "RESOLVED", "ARCHIVED", "ESCALATED"]) {
      const result = await updateConversationStatusSchema.safeParseAsync({
        status: s,
      });
      expect(result.success).toBe(true);
    }
...
describe("sendMessageSchema", () => {
  test("validates text required", async () => {
    expect(
      (await sendMessageSchema.safeParseAsync({ text: "Halo" })).success
    ).toBe(true);
    expect((await sendMessageSchema.safeParseAsync({ text: "" })).success).toBe(
      false
    );
...
describe("updateWebsiteSchema", () => {
  ...
  test("rejects invalid hex color", async () => {
    const r1 = await updateWebsiteSchema.safeParseAsync({
      primaryColor: "red",
    });
    expect(r1.success).toBe(false);
    const r2 = await updateWebsiteSchema.safeParseAsync({
      primaryColor: "#GGGGGG",
    });
